...
class TrivyScanner {
private:
    void create_directory_if_not_exists(const string &directory_path) {
        struct stat st{};
    ...
    void run_trivy_scan(const string &scan_type, const string &target, const string &directory, const string &filename) {
        ...
        fd << "VulnerabilityID,Severity,PkgName,PkgPath,InstalledVersion,FixedVersion,PublishedDate,LastModifiedDate\n";
                ...
                if (vulobject.isMember("Vulnerabilities")) {
                    for (const auto &typevulobject : vulobject["Vulnerabilities"]) {
                        fd << typevulobject["VulnerabilityID"].asString() << ","
                           ...
                           << typevulobject["PkgPath"].asString() << ","
...
public:
    void scan(const string &repo, const string &image) {
        ...
        if (!repo.empty()) {
            ...
            create_directory_if_not_exists("results/" + image);
            run_trivy_scan("i", image, image, image);
        } else {
